...
// ── Auth State ─────────────────────────────────────────────────────────────────
let token       = localStorage.getItem('dsaToken') || null;
let currentUser = JSON.parse(localStorage.getItem('dsaUser') || 'null');
...
function setAuth(user, tok) {
  token = tok;
  currentUser = user;
  localStorage.setItem('dsaToken', tok);
...
document.getElementById('loginTab').addEventListener('click', () => {
  ...
  document.getElementById('loginFormEl').style.display = '';
  document.getElementById('registerFormEl').style.display = 'none';
...
document.getElementById('registerTab').addEventListener('click', () => {
  ...
  document.getElementById('registerFormEl').style.display = '';
  document.getElementById('loginFormEl').style.display = 'none';
...
async function loadQuestions() {
  const raw = await api('/questions');
  questions = raw.map(fromAPI);
...
async function addQuestion() {
  ...
  try {
    ...
    showError(titleError, titleInput, err.message);
  } finally {
...
modalSave.addEventListener('click', async () => {
  ...
  if (!title) { showError(document.getElementById('editTitleError'), editTitleEl, 'Title is required'); return; }
  if (!topic) { showError(document.getElementById('editTopicError'), editTopicEl, 'Topic is required'); return; }
  ...
  try {
    const updated = await api(`/questions/${editingId}`, {
      method: 'PUT',
      body: JSON.stringify({
        title,
        topic,
    ...
    showError(document.getElementById('editTitleError'), editTitleEl, err.message);
  } finally {
...
function createCard(q) {
  ...
  let cls = 'question-card bg-white dark:bg-dsa-card p-4 rounded-2xl flex flex-col gap-2 hover:bg-slate-50 dark:hover:bg-dsa-accent transition-all border border-slate-200 dark:border-white/5 relative';
  if (q.solved) cls += ' solved';
  div.className = cls;
  div.dataset.id = q.id;
...
questionList.addEventListener('click', e => {
  ...
  if (tagBadge) {
    searchInput.value = tagBadge.dataset.tag;
    currentSearch = tagBadge.dataset.tag.toLowerCase();
    applySearchAndFilter();
    return;
  }
...
importFile.addEventListener('change', () => {
  ...
  reader.onload = e => {
    ...
    showConfirm(
      ...
      () => importQuestions(data, false),
      () => importQuestions(data, true),
      'Replace All',
      'Merge'
    );
  };
...
async function importQuestions(data, mergeOnly) {
  if (!mergeOnly) {
    await Promise.all(questions.map(q =>
      api(`/questions/${q.id}`, { method: 'DELETE' }).catch(() => {})
    ));
    questions = [];
  }

  const existingTitles = new Set(questions.map(q => q.title.toLowerCase()));
  const toImport = mergeOnly
    ? data.filter(q => !existingTitles.has(q.title.toLowerCase()))
    : data;
  ...
  const imported = results.filter(Boolean).map(fromAPI);
  questions = mergeOnly ? [...questions, ...imported] : imported;
  applySearchAndFilter();
  updateStats();
}
...
document.getElementById('bulkSolve').addEventListener('click', async () => {
  ...
  results.filter(Boolean).forEach(q => {
    ...
    questions = questions.map(eq => eq.id === norm.id ? norm : eq);
  });
  selectedIds.clear();
  applySearchAndFilter(); updateStats(); updateBulkBar();
});

document.getElementById('bulkUnsolve').addEventListener('click', async () => {
  const ids = [...selectedIds];
  if (!ids.length) return;
  const results = await Promise.all(
...
document.getElementById('bulkDelete').addEventListener('click', async () => {
  const ids = [...selectedIds];
  if (!ids.length) return;
  await Promise.all(ids.map(id => api(`/questions/${id}`, { method: 'DELETE' }).catch(() => {})));
